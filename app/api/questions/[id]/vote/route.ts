  ...
  try {
    ...
    if (existingVote) {
      if (existingVote.voteType === voteType) {
        ...
        await db.batch([
          db.delete(questionVotes).where(
            and(eq(questionVotes.questionId, questionId), eq(questionVotes.userId, userId!))
          ),
          db.update(questions)
        ...
        await db.batch([
          db.update(questionVotes)
            .set({ voteType: voteType as "upvote" | "downvote" })
            .where(and(eq(questionVotes.questionId, questionId), eq(questionVotes.userId, userId!))),
          db.update(questions)
            .set({ votes: sql`${questions.votes} + ${voteChange}` })
            .where(eq(questions.id, questionId)),
          db.update(userProfile)
