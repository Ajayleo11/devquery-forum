...
import { questions, user as userTable, answers } from "@/lib/db/schema";
...
export const sendAnswerAcceptedNotification = inngest.createFunction(
  {
    ...
    triggers:   { event: "answer.accepted" },
    ...
    const emailSent = await step.run("send-email", async () => {
      ...
      await resend.emails.send({
        ...
        html: `
          ...
          <p><a href="${process.env.NEXT_PUBLIC_APP_URL}/questions/${questionId}">View Your Answer</a></p>
        `,
      });
      return true;
    });
